...
export default class Crossroad extends Situation {
    constructor(canvas, userData, stamina, upgrades) {
        ...
        this.player = new Player(this.background.getXPos() + (this.background.getWidth() / 2) - ((this.background.getWidth() / 8) / 2), this.background.getWidth() / 1.2, 0, 0, this.background.getWidth() / 20, this.background.getHeight() / 8, stamina);
    }
