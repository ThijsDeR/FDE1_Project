...
export default class PrioritySameRoad extends Situation {
    constructor(canvas, userData, playerData, upgrades, skins, keyListener, allowedMist) {
        ...
        Game.randomInteger(0, 5) === 1 ? '' : this.props.push(new TrackProp(cycleVectors, 
        // Prop image properties
        this.background.getWidth() / 20, this.background.getHeight() / 8, './assets/img/players/cycles/fiets1normal.png'));
        Game.randomInteger(0, 2) === 1 ? this.props.push(new StaminaBooster(this.background.getXPos() + ((this.background.getWidth() / 3) * 2) - (this.background.getWidth() / 30), this.background.getYPos() + ((this.background.getHeight() / 3) * 2), 0, 0, this.background.getWidth() / 16, this.background.getHeight() / 9, this.skins.staminaSkin.src, parseInt(this.skins.staminaSkin.baseStamina))) : '';
    }
}
