...
export default class OncomingCyclist {
    constructor(canvas) {
        this.background = new ImageProp(0, -canvas.height, 0, 0, canvas.width, canvas.height, './assets/img/objects/KruispuntGeenZebrapad.png');
        this.props = [
    ...
    update(elapsed, scrollSpeed, player) {
        ...
        if (player.getYPos() < this.background.getYPos() - this.background.getHeight()) {
            return OncomingCyclist.FINISHED;
        }
        let gameOver = false;
        ...
        return gameOver ? OncomingCyclist.GAME_OVER : OncomingCyclist.NOT_DONE;
    }
    draw(ctx) {
...
OncomingCyclist.NOT_DONE = 0;
OncomingCyclist.GAME_OVER = 1;
OncomingCyclist.FINISHED = 2;
